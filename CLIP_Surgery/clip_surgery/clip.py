...
def similarity_map_to_points(sm, shape, t=0.8, down_sample=2):
    ...
    for idx in rank[-num:]:
        ...
        idx_val = to_int(idx)
        x = min((idx_val % w + 0.5) * scale_w, shape[1] - 1) # +0.5 to center
        y = min((idx_val // w + 0.5) * scale_h, shape[0] - 1)
        points.append([to_int(x), to_int(y)])

    # negatives
    for idx in rank[:num]:
        # Convert idx to int if it's a tensor
        idx_val = to_int(idx)
        x = min((idx_val % w + 0.5) * scale_w, shape[1] - 1)
        y = min((idx_val // w + 0.5) * scale_h, shape[0] - 1)
        points.append([to_int(x), to_int(y)])
